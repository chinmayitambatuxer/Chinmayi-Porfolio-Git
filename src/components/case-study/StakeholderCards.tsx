...
function StakeholderCard({
  ...
  return (
    <article
      className={`flex h-full min-w-0 flex-col rounded-2xl border p-5 transition-shadow duration-200 hover:shadow-md md:p-6 ${className}`}
      style={{
    ...
    >
      ...
      >
        <div>
          ...
          <p className="mt-1.5 text-sm leading-relaxed text-ink">{primaryGoal}</p>
        </div>
        <div>
          <p className="cs-meta-label" style={{ color: theme.accent }}>
            UX consideration
...
export function StakeholderCards({ rows, theme }: StakeholderCardsProps) {
  return (
    <>
      ...
      <div className="mt-10 hidden gap-5 sm:grid-cols-2 lg:grid lg:grid-cols-3">
        {rows.map(([stakeholder, primaryGoal, uxConsideration], index) => (
          <StakeholderCard
            key={stakeholder}
            stakeholder={stakeholder}
            primaryGoal={primaryGoal}
            uxConsideration={uxConsideration}
            index={index}
            theme={theme}
          />
        ))}
